    ...
    return (
        <div className="flex flex-col items-center justify-center w-max mx-auto p-4 space-y-4">
            <div className="grid grid-cols-8 gap-4 items-center w-full text-white text-lg py-4 font-medium ">
                ...
                <div className="bg-bgcolor-primary rounded-2xl py-4 text-center drop-shadow-bg col-span-3 mx-1">Name</div>
            ...
            {data.map((stock) => {
                return (
                    <Link key={"stock_list_" + stock} href={"/stocks/" + stock.ticker} className="flex flex-row w-full items-center group justify-end cursor-pointer">
                        <div className="grid grid-cols-8 gap-4 w-full items-center bg-bgcolor-primary drop-shadow-bg-light rounded-2xl py-4 transition-all duration-200 group-hover:-translate-x-5 group-hover:bg-accent">
                            ...
                            <div className={`justify-between mx-4 text-white text-lg font-light text-center rounded-full px-4 py-2 drop-shadow-bg-light ${parseFloat(stock.change.replaceAll("%", "")) > 0 ? "bg-gain-s" : "bg-loss-s"}`}>
                                {stock.change}
                            </div>
