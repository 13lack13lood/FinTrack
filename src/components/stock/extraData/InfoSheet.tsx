		...
		<div className="flex flex-col w-full items-center justify-center space-y-7">
			<div className="flex flex-wrap items-center justify-center space-x-4">
				{data.map((value, index) => {
					return (
						<div
							className={`${
								selected == index ? "bg-accent" : "bg-bgcolor-primary"
							} text-white font-light rounded-full px-5 py-2 drop-shadow-bg transition-all duration-200 hover:bg-accent hover:opacity-[85%] ease-out`}
			...
			<div className="flex flex-col w-max bg-bgcolor-primary space-y-4 p-4 rounded-2xl drop-shadow-bg">
				{Object.entries(data[selected][1]).map(([key, value]) => {
					return (
						<div className="flex flex-row justify-between space-x-8 text-white bg-bgcolor-secondary drop-shadow-bg-light px-3 py-1 rounded-full">
